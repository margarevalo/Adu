...
function assertArgsArraysEqual(context, Assertion, chaiUtils, expectedArgs, log, assert, ssfi) {
    ...
    for (let index = 0; index < expectedArgs.length; index++) {
        if (typeof expectedArgs[index] === "function") {
            const errorPrefix = `The predicate for the ${ordinal(index + 1)} event argument`;
            try {
            ...
            assert(expectedLength === actualLength, `Expected the ${ordinal(index + 1)} argument of the "${eventName}" event to have ${expectedLength} ${expectedLength === 1 ? "element" : "elements"}, but it has ${actualLength}`);
